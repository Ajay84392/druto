...
<div class="flex-1 overflow-auto p-6 md:p-10">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-[#0f172a] mb-1">Edit Coupon</h1>
        <div class="text-xs text-[#475569] font-medium flex items-center space-x-1">
            ...
            <svg class="w-3 h-3 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"></path></svg>
            <span class="text-[#0f172a]">Edit</span>
        </div>
    </div>
        ...
        <form action="{{ route('coupons.update', $coupon->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
                <div>
                    ...
                    <input type="text" name="code" value="{{ old('code', $coupon->code) }}" class="w-full border border-[#e2e8f0] rounded-xl px-4 py-2 focus:ring-red-500 focus:border-red-500 uppercase">
            ...
            <div class="mb-4">
                ...
                <textarea name="description" rows="2" class="w-full border border-[#e2e8f0] rounded-xl px-4 py-2 focus:ring-red-500 focus:border-red-500">{{ old('description', $coupon->description) }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="discount_value" value="{{ old('discount_value', $coupon->discount_value) }}" class="w-full border border-[#e2e8f0] rounded-xl px-4 py-2 focus:ring-red-500 focus:border-red-500">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
                <div>
                    ...
                    <input type="number" step="0.01" name="min_order_value" value="{{ old('min_order_value', $coupon->min_order_value) }}" class="w-full border border-[#e2e8f0] rounded-xl px-4 py-2 focus:ring-red-500 focus:border-red-500">
                ...
                <div>
                    ...
                    <input type="number" name="max_uses" value="{{ old('max_uses', $coupon->max_uses) }}" class="w-full border border-[#e2e8f0] rounded-xl px-4 py-2 focus:ring-red-500 focus:border-red-500">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="date" name="starts_at" value="{{ old('starts_at', $coupon->starts_at) ? \Carbon\Carbon::parse(old('starts_at', $coupon->starts_at))->format('Y-m-d') : '' }}" class="w-full border border-[#e2e8f0] rounded-xl px-4 py-2 focus:ring-red-500 focus:border-red-500">
                ...
                <div>
                    ...
                    <input type="date" name="expires_at" value="{{ old('expires_at', $coupon->expires_at) ? \Carbon\Carbon::parse(old('expires_at', $coupon->expires_at))->format('Y-m-d') : '' }}" class="w-full border border-[#e2e8f0] rounded-xl px-4 py-2 focus:ring-red-500 focus:border-red-500">
                    @error('expires_at') <span class="text-[#EF4444] text-xs">{{ $message }}</span> @enderror
                </div>
            </div>

            <button type="submit" class="bg-[#b00000] text-white px-6 py-2 rounded-xl font-bold hover:bg-[#8a0000] transition">Update Coupon</button>
