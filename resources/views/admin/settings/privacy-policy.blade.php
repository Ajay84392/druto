...
<div class="flex-1 overflow-auto p-6 md:p-10">
    ...
    <form id="policyForm" action="{{ url('/admin/settings') }}" method="POST" class="bg-white rounded-2xl shadow-sm border border-[#e2e8f0]">
        @csrf
        <input type="hidden" name="group" value="policy">
        <input type="hidden" name="privacy_policy_content" id="policyContent">
        <input type="hidden" name="action" id="formAction" value="publish">
        
        <!-- Tabs -->
        <div class="flex border-b border-[#e2e8f0] px-6 pt-4">
            <a href="/admin/settings/privacy-policy" class="px-6 py-3 border-b-2 border-[#b00000] text-[#b00000] font-bold text-sm">Privacy Policy</a>
            <a href="/admin/settings/terms" class="px-6 py-3 border-b-2 border-transparent text-[#475569] hover:text-slate-700 font-bold text-sm transition">Terms & Conditions</a>
        </div>
        ...
        <div class="p-6 md:p-8 flex flex-col lg:flex-row gap-8 lg:gap-12">
            ...
            <div class="flex-1 flex flex-col min-w-0">
                ...
                <div class="flex-1 flex flex-col border border-[#e2e8f0] rounded-xl overflow-hidden bg-white">
                    <div id="editor-container" class="h-96">{!! $settings['privacy_policy_content'] ?? '' !!}</div>
                </div>
            ...
            <div class="w-full lg:w-72 flex-shrink-0 space-y-8">
                ...
                <div class="bg-[#f1f5f9] rounded-xl border border-slate-100 p-5 space-y-5">
                    ...
                    <div>
                        ...
                        <div class="text-xs font-semibold text-[#0f172a]">{{ isset($settings['privacy_policy_last_updated']) ? \Carbon\Carbon::parse($settings['privacy_policy_last_updated'])->format('M d, Y h:i A') : '-' }}</div>
                    </div>
                    
                    <div>
                        <div class="text-[10px] uppercase font-bold tracking-wider text-[#475569] mb-1">Version</div>
                        <div class="text-xs font-semibold text-[#0f172a]">{{ $settings['privacy_policy_version'] ?? '1.0' }}</div>
                    </div>
                    
                    <div>
                        <div class="text-[10px] uppercase font-bold tracking-wider text-[#475569] mb-1">Published By</div>
                        <div class="text-xs font-semibold text-[#0f172a]">{{ $settings['privacy_policy_published_by'] ?? '-' }}</div>
                    </div>
                    
                    <div>
                        <div class="text-[10px] uppercase font-bold tracking-wider text-[#475569] mb-1">Published On</div>
                        <div class="text-xs font-semibold text-[#0f172a]">{{ isset($settings['privacy_policy_published_on']) ? \Carbon\Carbon::parse($settings['privacy_policy_published_on'])->format('M d, Y h:i A') : '-' }}</div>
                    </div>
                </div>
...
<div id="previewModal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4" style="background:rgba(0,0,0,0.6)">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col">
        <div class="flex items-center justify-between px-6 py-4 border-b border-[#e2e8f0]">
            <h2 class="text-lg font-black text-[#0f172a]">Privacy Policy Preview</h2>
