...
<div class="flex-1 overflow-auto p-6 md:p-10">
    <div class="mb-8">
        <h1 class="text-2xl font-bold text-[#0f172a] mb-1">Edit Referral</h1>
    ...
    <form action="{{ url('/admin/referrals/' . $referral->id) }}" method="POST" class="bg-white rounded-2xl shadow-sm border border-[#e2e8f0] p-6 md:p-8 max-w-3xl">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
                ...
                <div class="flex-1">
                    <select name="referrer_id" required class="w-full px-4 py-2.5 rounded-xl border border-[#e2e8f0] focus:outline-none focus:ring-1 focus:ring-red-500 focus:border-red-500 text-sm font-semibold text-[#0f172a] bg-white">
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ old('referrer_id', $referral->referrer_id) == $user->id ? 'selected' : '' }}>{{ $user->name }} ({{ $user->email }})</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="flex flex-col md:flex-row md:items-center gap-2 md:gap-6">
                ...
                <div class="flex-1">
                    <input type="email" name="referred_email" value="{{ old('referred_email', $referral->referred_email) }}" placeholder="email@example.com" class="w-full px-4 py-2.5 rounded-xl border border-[#e2e8f0] focus:outline-none focus:ring-1 focus:ring-red-500 focus:border-red-500 text-sm font-semibold text-[#0f172a]">
                </div>
            </div>
            ...
            <div class="flex flex-col md:flex-row md:items-center gap-2 md:gap-6">
                ...
                <div class="flex-1">
                    <input type="number" name="reward_coins" value="{{ old('reward_coins', $referral->reward_coins) }}" min="0" required class="w-full px-4 py-2.5 rounded-xl border border-[#e2e8f0] focus:outline-none focus:ring-1 focus:ring-red-500 focus:border-red-500 text-sm font-semibold text-[#0f172a]">
                </div>
            </div>
            ...
            <div class="flex flex-col md:flex-row md:items-center gap-2 md:gap-6">
                ...
                <div class="flex-1">
                    <input type="datetime-local" name="expires_at" value="{{ old('expires_at', $referral->expires_at ? date('Y-m-d\TH:i', strtotime($referral->expires_at)) : null) }}" class="w-full px-4 py-2.5 rounded-xl border border-[#e2e8f0] focus:outline-none focus:ring-1 focus:ring-red-500 focus:border-red-500 text-sm font-semibold text-[#0f172a]">
                </div>
            </div>
        </div>
        ...
        <div class="mt-8 flex items-center justify-end space-x-3 pt-6 border-t border-slate-100">
            ...
            <button type="submit" class="px-6 py-2.5 bg-[#b00000] text-white text-sm font-bold rounded-xl hover:bg-[#8a0000] transition">Edit Referral</button>
        </div>
    </form>
</div>
@endsection
