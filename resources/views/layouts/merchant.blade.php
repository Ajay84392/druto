...
<body class="bg-[#f1f5f9] text-[#0f172a] antialiased h-[100dvh] overflow-hidden flex relative" x-data="{ sidebarOpen: false, logoutModal: false }">
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed md:static inset-y-0 left-0 w-[240px] bg-[#b00000] text-white flex flex-col flex-shrink-0 h-full overflow-y-auto shadow-2xl z-50 transform md:translate-x-0 transition-transform duration-200 ease-in-out">
        ...
        <div class="h-[72px] flex items-center px-6">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    ...
                    <div class="text-[11px] font-medium text-red-200">Merchant Panel</div>
                </div>
            </div>
        </div>
        ...
        <nav class="flex-1 px-4 py-6 space-y-1">
            <a href="/merchant"
                class="flex items-center space-x-3 px-4 py-3 rounded-xl text-sm font-semibold transition {{ request()->is('merchant') || request()->is('merchant/dashboard') ? 'bg-[#8a0000] text-white' : 'text-red-100 hover:bg-[#8a0000] hover:text-white' }}">
            ...
            <a href="/merchant/rewards"
                class="flex items-center space-x-3 px-4 py-3 rounded-xl text-sm font-semibold transition {{ request()->is('merchant/rewards') && !request()->is('merchant/rewards/create*') ? 'bg-[#8a0000] text-white' : 'text-red-100 hover:bg-[#8a0000] hover:text-white' }}">
                <svg class="w-5 h-5 {{ request()->is('merchant/rewards') && !request()->is('merchant/rewards/create*') ? 'text-white' : '' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8v13m0-13V6a2 2 0 112 2h-2zm0 0V5.5A2.5 2.5 0 109.5 8H12zm-7 4h14M5 12a2 2 0 110-4h14a2 2 0 110 4M5 12v7a2 2 0 002 2h10a2 2 0 002-2v-7"></path></svg>
                <span>Rewards</span>
            </a>


            <a href="/merchant/create-offer"
                class="flex items-center space-x-3 px-4 py-3 rounded-xl text-sm font-semibold transition {{ request()->is('merchant/create-offer*') ? 'bg-[#8a0000] text-white' : 'text-red-100 hover:bg-[#8a0000] hover:text-white' }}">
                <svg class="w-5 h-5 {{ request()->is('merchant/create-offer*') ? 'text-white' : '' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                <span>Create Offer</span>
            </a>

            <a href="/merchant/profile"
                class="flex items-center space-x-3 px-4 py-3 rounded-xl text-sm font-semibold transition {{ request()->is('merchant/profile*') ? 'bg-[#8a0000] text-white' : 'text-red-100 hover:bg-[#8a0000] hover:text-white' }}">
    ...
    <main class="flex-1 flex flex-col h-full bg-[#f1f5f9] overflow-hidden w-full relative">
        ...
        <header class="hidden md:flex h-[72px] bg-white border-b border-[#e2e8f0] items-center justify-between px-6 flex-shrink-0 w-full">
            ...
            <div class="flex items-center space-x-6 ml-auto">
                <!-- Profile Link -->
                <a href="/merchant/profile" class="flex items-center space-x-3 select-none bg-[#f1f5f9] hover:bg-slate-100 border border-[#e2e8f0] px-3 py-1.5 rounded-full transition">
                    @php
                        $nameParts = explode(' ', auth()->user()->name ?? 'Merchant');
        ...
        <!-- View Content -->
        <div class="w-full max-w-[1600px] mx-auto md:p-8 overflow-auto flex-1">
            @yield('content')
        </div>
    </main>

    <!-- Mobile Bottom Navigation (Visible only on mobile) -->
    <nav class="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-[#e2e8f0] z-50 px-6 py-2 shadow-[0_-4px_20px_-5px_rgba(0,0,0,0.05)] pb-safe">
        <div class="flex justify-between items-center relative">
            
            <a href="/merchant" class="flex flex-col items-center p-2 flex-1 {{ request()->is('merchant') || request()->is('merchant/dashboard') ? 'text-[#b00000]' : 'text-slate-400' }}">
            ...
            <a href="/merchant/rewards" class="flex flex-col items-center p-2 flex-1 {{ request()->is('merchant/rewards') ? 'text-[#b00000]' : 'text-slate-400' }}">
                <svg class="w-6 h-6 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8v13m0-13V6a2 2 0 112 2h-2zm0 0V5.5A2.5 2.5 0 109.5 8H12zm-7 4h14M5 12a2 2 0 110-4h14a2 2 0 110 4M5 12v7a2 2 0 002 2h10a2 2 0 002-2v-7"></path></svg>
                <span class="text-[10px] font-bold tracking-wide">Rewards</span>
            </a>


            <a href="/merchant/create-offer" class="flex flex-col items-center p-2 flex-1 {{ request()->is('merchant/create-offer*') ? 'text-[#b00000]' : 'text-slate-400' }}">
                <svg class="w-6 h-6 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                <span class="text-[10px] font-bold tracking-wide">Create Offer</span>
            </a>
