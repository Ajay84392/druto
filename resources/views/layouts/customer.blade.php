...
<body class="bg-slate-50 text-slate-800 antialiased h-screen overflow-hidden flex">
    ...
    <aside class="w-64 bg-[#b00000] text-white flex flex-col flex-shrink-0 h-full overflow-y-auto">
        ...
        <nav class="flex-1 px-4 py-6 space-y-1">
            <a href="/customer" class="flex items-center space-x-3 px-4 py-3 rounded-lg text-sm font-semibold transition {{ request()->is('customer') || request()->is('customer/dashboard') ? 'bg-[#8a0000] text-white' : 'text-red-100 hover:bg-[#9a0000] hover:text-white' }}">
            ...
            <a href="/customer/scan" class="flex items-center space-x-3 px-4 py-3 rounded-lg text-sm font-semibold transition {{ request()->is('customer/scan*') ? 'bg-[#8a0000] text-white' : 'text-red-100 hover:bg-[#9a0000] hover:text-white' }}">
                <svg class="w-5 h-5 {{ request()->is('customer/scan*') ? 'text-white' : '' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"></path></svg>
                <span>Scan QR</span>
            </a>

            <a href="/customer/rewards" class="flex items-center space-x-3 px-4 py-3 rounded-lg text-sm font-semibold transition {{ request()->is('customer/reward*') ? 'bg-[#8a0000] text-white' : 'text-red-100 hover:bg-[#9a0000] hover:text-white' }}">
                <svg class="w-5 h-5 {{ request()->is('customer/reward*') ? 'text-white' : '' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8v13m0-13V6a2 2 0 112 2h-2zm0 0V5.5A2.5 2.5 0 109.5 8H12zm-7 4h14M5 12a2 2 0 110-4h14a2 2 0 110 4M5 12v7a2 2 0 002 2h10a2 2 0 002-2v-7"></path></svg>
                <span>Rewards</span>
            </a>

            <a href="/customer/profile" class="flex items-center space-x-3 px-4 py-3 rounded-lg text-sm font-semibold transition {{ request()->is('customer/profile*') ? 'bg-[#8a0000] text-white' : 'text-red-100 hover:bg-[#9a0000] hover:text-white' }}">
                <svg class="w-5 h-5 {{ request()->is('customer/profile*') ? 'text-white' : '' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                <span>Profile</span>
            </a>
        </nav>
        ...
        <header class="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 flex-shrink-0">
            ...
            <div class="flex items-center space-x-6">
                ...
                <div class="flex items-center space-x-3 cursor-pointer">
                    <img src="https://ui-avatars.com/api/?name=Customer&background=0D8ABC&color=fff" alt="Customer" class="w-9 h-9 rounded-full object-cover">
                    <div class="hidden md:block text-right">
                        <div class="text-sm font-bold text-slate-900 leading-tight">Customer</div>
                        <div class="text-[11px] font-semibold text-slate-500">Member</div>
                    </div>
